...
void METSignificanceInputProducer::produce(edm::StreamID streamID,
                                           edm::Event& iEvent,
  ...
  out->addColumnValue<float>("sumPtUnclustEnDeltaUp",
                             sumPtUp,
                             "sumPt, unclustered Energy up (MET_sumPt_mod-MET_sumPt)",
                             nanoaod::FlatTable::FloatColumn);
  iEvent.put(std::move(out));
}

bool METSignificanceInputProducer::cleanJet(const pat::Jet& jet,
                                            const std::vector<edm::Handle<reco::CandidateView>>& leptons) const {
  for (const auto& lep_i : leptons) {
    for (const auto& lep : *lep_i) {
      if (lep.pt() > 0) {
